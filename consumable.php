...
function view_edit_consumable_receipt($link,$consumable_name,$edit_id=0,$save_mode='no')
{
	...
	//$sql='select * from consumable_receipt where consumable_name=\''.$consumable_name.'\' order by id desc limit 500';
	$sql='select * from consumable_receipt where consumable_name=\''.$consumable_name.'\' order by id DESC limit 1000';
	if(!$result=run_query($link,$GLOBALS['database'],$sql))
	{
		 return false;
	}
